...
const LABELS: Record<string, string> = { proveedor: "Proveedor", producto: "Producto", familia: "Familia" };
...
function ComprasContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch<Reporte>(`/reportes/compras-agrupacion?${p}`),
  ...
  useEffect(() => {
    if (reporte) document.title = `Compras por ${reporte.agrupar_por} ${reporte.fecha_desde} ${reporte.fecha_hasta}`;
  }, [reporte]);
  ...
  const mostrarCantidad = reporte.agrupar_por !== "proveedor";
  const mostrarId = reporte.agrupar_por !== "familia";
  const idHeader = reporte.agrupar_por === "proveedor" ? "NIT" : "Código";
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "2px solid #1e293b" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ border: "2px solid #1e293b", borderRadius: 8, padding: "10px 18px", display: "inline-block", minWidth: 190 }}>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "#64748b" }}>Compras por {colLabel}</div>
            </div>
...
export default function ComprasAgrupacionPrint() {
  return (
