...
function AuxiliarContent() {
  ...
  const centroId   = params.get("centro_costo_id") ?? "";
  const incluirHijos = params.get("incluir_hijos") ?? "";
  ...
  useEffect(() => {
    ...
    if (centroId) { p.set("centro_costo_id", centroId); if (incluirHijos) p.set("incluir_hijos", incluirHijos); }
    apiFetch<Auxiliar>(`/reportes/auxiliar-centro-costo?${p}`).then(setData);
    apiFetch<Empresa>("/empresa").catch(() => null).then(e => { if (e) setEmpresa(e); });
  }, [cDesde, cHasta, fechaDesde, fechaHasta, centroId, incluirHijos]);

  useEffect(() => { if (data) document.title = `Auxiliar por Centro de Costo ${data.fecha_desde} ${data.fecha_hasta}`; }, [data]);
    ...
    <>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "20px 28px", fontSize: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, paddingBottom: 12, borderBottom: "2px solid #1e293b" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ border: "2px solid #1e293b", borderRadius: 7, padding: "10px 20px", display: "inline-block" }}>
              <div style={{ fontSize: 8, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "#64748b" }}>Auxiliar por Centro de Costo</div>
            </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              <th style={{ ...th, textAlign: "left", width: 66 }}>Fecha</th>
              <th style={{ ...th, textAlign: "left", width: 50 }}>Asiento</th>
              <th style={{ ...th, textAlign: "left" }}>Cuenta</th>
              <th style={{ ...th, textAlign: "left" }}>Centro de costo</th>
              <th style={{ ...th, textAlign: "left" }}>Descripción</th>
              <th style={{ ...thR, width: 85 }}>Débito</th>
              <th style={{ ...thR, width: 85 }}>Crédito</th>
            </tr>
          </thead>
          <tbody>
            {data.movimientos.map((m, i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                ...
                <td style={{ ...td, fontFamily: "monospace", textAlign: "center" }}>{m.numero}</td>
                <td style={td}><span style={{ fontFamily: "monospace", fontWeight: 700 }}>{m.cuenta_codigo}</span> {m.cuenta_nombre}</td>
                <td style={td}>{m.centro_costo_codigo ? <><span style={{ fontFamily: "monospace", fontWeight: 700 }}>{m.centro_costo_codigo}</span> {m.centro_costo_nombre}</> : "—"}</td>
                <td style={td}>{m.descripcion}</td>
                <td style={tdR}>{fmt(m.debito)}</td>
                <td style={tdR}>{fmt(m.credito)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr style={{ borderTop: "2px solid #94a3b8", background: "#f1f5f9" }}>
              <td colSpan={5} style={{ ...tdR, fontWeight: 700, fontSize: 8, textTransform: "uppercase" }}>Totales</td>
              <td style={{ ...tdR, fontWeight: 700 }}>{fmt(data.totales.debito)}</td>
              <td style={{ ...tdR, fontWeight: 700 }}>{fmt(data.totales.credito)}</td>
            </tr>
          </tfoot>
        </table>
...
export default function ImprimirAuxiliarCcPage() {
