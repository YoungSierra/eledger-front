...
function SaldosContent() {
  ...
  useEffect(() => {
    apiFetch<ResumenResponse>(`/cxp/resumen?fecha_corte=${fechaCorte}`).then(setData);
    apiFetch<Empresa>("/empresa").catch(() => null).then((e) => { if (e) setEmpresa(e); });
  }, [fechaCorte]);

  useEffect(() => {
    if (data) document.title = `Saldos Proveedores ${fechaCorte}`;
  }, [data, fechaCorte]);
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "2px solid #1e293b" }}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ border: "2px solid #1e293b", borderRadius: 8, padding: "10px 18px", display: "inline-block", minWidth: 200, textAlign: "right" }}>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "#64748b" }}>Saldos de Proveedores</div>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "#64748b", marginTop: 2 }}>Cuentas por Pagar</div>
            </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              ...
              <th style={{ ...thLeft, minWidth: 180 }}>Proveedor</th>
          ...
          <tfoot>
            <tr style={{ borderTop: "2px solid #1e293b" }}>
              <td colSpan={2} style={{ ...tdLeft, fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5 }}>Total general</td>
...
export default function ImprimirSaldosProveedoresPage() {
  return (
