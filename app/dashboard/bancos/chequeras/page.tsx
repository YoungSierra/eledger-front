...
export default function ChequerasPage() {
  ...
  const { orden, alternar } = useOrden<
    "banco" | "prefijo" | "rango" | "consecutivo" | "estado"
  >("rango", "asc", () => setPagina(1));
  ...
  const ordenada = ordenarFilas(filtrados, orden, {
    ...
    prefijo:     (c) => c.prefijo,
    rango:       (c) => c.numero_desde,
    consecutivo: (c) => c.consecutivo_actual,
    estado:      (c) => c.estado,
  });
          ...
          <table className="w-full min-w-[760px]">
            <thead className="sticky top-0 bg-gray-50/70 z-10">
              <tr className="border-b border-gray-100">
                <Th campo="banco"       orden={orden} alternar={alternar}>Banco / Cuenta</Th>
                <Th campo="prefijo"     orden={orden} alternar={alternar}>Prefijo</Th>
                <Th campo="rango"       orden={orden} alternar={alternar}>Rango</Th>
                <Th campo="consecutivo" orden={orden} alternar={alternar}>Consecutivo</Th>
                ...
                <tr key={c.id} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      <div>
                        <p className="text-[12px] text-blue-600 font-semibold">{c.banco_nombre ?? "—"}</p>
      ...
      {drawer && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full sm:w-[440px] bg-white shadow-xl z-50 flex flex-col">
            <DrawerHeader
              title={drawer === "crear" ? "Nueva chequera" : "Editar chequera"}
