...
export default function BancosPage() {
  ...
  const { orden, alternar } = useOrden<
    "nombre" | "codigo" | "nit" | "extracto" | "estado"
          ...
          <table className="w-full min-w-[760px]">
            <thead className="sticky top-0 bg-gray-50/70 z-10">
              <tr className="border-b border-gray-100">
                ...
                <Th campo="nit"      orden={orden} alternar={alternar}>NIT</Th>
                <Th campo="extracto" orden={orden} alternar={alternar}>Extracto</Th>
                ...
                <tr key={b.id} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-4 py-3 text-[12px]">
                    <div className="flex items-center gap-2.5">
                      <span className="font-semibold text-blue-600">{b.nombre}</span>
                    </div>
                  </td>
      ...
      {drawer && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full sm:w-[440px] bg-white shadow-xl z-50 flex flex-col">
            <DrawerHeader
              title={drawer === "crear" ? "Nuevo banco" : "Editar banco"}
