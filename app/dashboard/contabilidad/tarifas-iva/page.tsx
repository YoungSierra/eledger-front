...
interface TarifaIva {
  ...
  cuenta_iva_compras_id: string | null;
  cuenta_iva_compras_codigo: string | null;
  cuenta_iva_compras_nombre: string | null;
  activo: boolean;
}

interface Cuenta { id: string; codigo: string; nombre: string; }

const TIPO_LABEL: Record<string, string> = { GRAVADO: "Gravado", EXENTO: "Exento", EXCLUIDO: "Excluido" };
const TIPO_STYLE: Record<string, string> = {
  GRAVADO:  "bg-blue-50 text-blue-700",
  EXENTO:   "bg-green-50 text-green-700",
...
const EMPTY = {
  nombre: "", tipo: "GRAVADO", porcentaje: "0",
  cuenta_iva_ventas_id: "", cuenta_iva_ventas_display: "",
  cuenta_iva_compras_id: "", cuenta_iva_compras_display: "",
};
...
export default function TarifasIvaPage() {
  const title = usePageTitle();
  const [lista, setLista]       = useState<TarifaIva[]>([]);
  ...
  const { orden, alternar } = useOrden<
    "nombre" | "tipo" | "porcentaje" | "cuenta_ventas" | "cuenta_compras" | "estado"
  >("tipo", "asc", () => setPagina(1));

  const [drawer, setDrawer]     = useState<"crear" | "editar" | null>(null);
  const [editando, setEditando] = useState<TarifaIva | null>(null);
  ...
  async function cargar() {
    setLoading(true);
    try {
      const data = await apiFetch<TarifaIva[]>(`/maestros/tarifas-iva?solo_activas=${soloActivas}`);
  ...
  function abrirEditar(t: TarifaIva) {
    setForm({
      nombre: t.nombre, tipo: t.tipo,
      porcentaje: String(parseFloat(t.porcentaje)),
      cuenta_iva_ventas_id: t.cuenta_iva_ventas_id ?? "",
      cuenta_iva_ventas_display: t.cuenta_iva_ventas_id ? `${t.cuenta_iva_ventas_codigo} — ${t.cuenta_iva_ventas_nombre}` : "",
      cuenta_iva_compras_id: t.cuenta_iva_compras_id ?? "",
      cuenta_iva_compras_display: t.cuenta_iva_compras_id ? `${t.cuenta_iva_compras_codigo} — ${t.cuenta_iva_compras_nombre}` : "",
    });
    setEditando(t); setError(""); setDrawer("editar");
  }
  ...
  async function guardar() {
    ...
    try {
      const body = {
        ...
        cuenta_iva_ventas_id: form.cuenta_iva_ventas_id || null,
        cuenta_iva_compras_id: form.cuenta_iva_compras_id || null,
      };
      if (drawer === "crear") {
        await apiFetch("/maestros/tarifas-iva", { method: "POST", body: JSON.stringify(body) });
      } else {
        await apiFetch(`/maestros/tarifas-iva/${editando!.id}`, { method: "PUT", body: JSON.stringify(body) });
      }
  ...
  async function toggleActivo(t: TarifaIva) {
    try {
      await apiFetch(`/maestros/tarifas-iva/${t.id}`, { method: "PUT", body: JSON.stringify({ activo: !t.activo }) });
      await cargar();
    } catch {}
  }

  const filtradas = lista.filter((t) => t.nombre.toLowerCase().includes(busqueda.toLowerCase()));
  // Se ordena la lista completa antes de paginar.
  const ordenada = ordenarFilas(filtradas, orden, {
    nombre:         (t) => t.nombre,
    tipo:           (t) => TIPO_LABEL[t.tipo] ?? t.tipo,
    porcentaje:     (t) => Number(t.porcentaje),
    cuenta_ventas:  (t) => t.cuenta_iva_ventas_codigo,
    cuenta_compras: (t) => t.cuenta_iva_compras_codigo,
    ...
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-5 shrink-0">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">Tarifas de impuesto al valor agregado vinculadas al plan de cuentas</p>
        </div>
          ...
          <table className="w-full min-w-[760px]">
            <thead className="sticky top-0 bg-gray-50/70 z-10">
              <tr className="border-b border-gray-100">
                ...
                <Th campo="porcentaje"     orden={orden} alternar={alternar}>% IVA</Th>
                ...
                <tr key={t.id} className="hover:bg-blue-50/30 transition-colors">
                  <td className="px-4 py-3 text-[12px]">
                    <div className="flex items-center gap-2.5">
                      <span className="font-semibold text-blue-600">{t.nombre}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${TIPO_STYLE[t.tipo] ?? "bg-gray-100 text-gray-500"}`}>
                      {TIPO_LABEL[t.tipo] ?? t.tipo}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-gray-700 font-semibold">{parseFloat(t.porcentaje)}%</td>
                  <td className="px-4 py-3 text-[11px] text-gray-500 font-mono">{t.cuenta_iva_ventas_codigo ?? "—"}</td>
                  <td className="px-4 py-3 text-[11px] text-gray-500 font-mono">{t.cuenta_iva_compras_codigo ?? "—"}</td>
                  <td className="px-4 py-3">
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${t.activo ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 justify-end">
                      ...
                      <button onClick={() => toggleActivo(t)} title={t.activo ? "Inactivar" : "Activar"}
                        className={`p-1.5 rounded-md transition-colors ${t.activo ? "text-gray-400 hover:text-red-500 hover:bg-red-50" : "text-gray-400 hover:text-green-600 hover:bg-green-50"}`}>
                        {t.activo
      ...
      {drawer && (
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-full sm:w-[440px] bg-white shadow-xl z-50 flex flex-col">
            <DrawerHeader
              title={drawer === "crear" ? "Nueva tarifa de IVA" : "Editar tarifa de IVA"}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div>
                ...
                <input value={form.nombre} onChange={(e) => setForm(p => ({ ...p, nombre: e.target.value }))}
                  placeholder="IVA 19%, IVA 5%, Exento..." className={inputCls} />
              </div>
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400 mt-1">Para Exento y Excluido usar 0</p>
              </div>

              <div className="border-t border-gray-100 pt-4">
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-3">Cuentas PUC</p>
                <div className="space-y-3">
                  <CuentaSearch label="Cuenta IVA ventas (generado)"
                    cuentaId={form.cuenta_iva_ventas_id}
                    cuentaDisplay={form.cuenta_iva_ventas_display}
                    onChange={(id) => setForm(p => ({ ...p, cuenta_iva_ventas_id: id }))} />
                  <CuentaSearch label="Cuenta IVA compras (descontable)"
                    cuentaId={form.cuenta_iva_compras_id}
                    cuentaDisplay={form.cuenta_iva_compras_display}
                    onChange={(id) => setForm(p => ({ ...p, cuenta_iva_compras_id: id }))} />
                </div>
              </div>
            </div>
