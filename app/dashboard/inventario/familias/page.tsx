    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        ...
        <label className="flex items-center gap-2 text-[12px] text-gray-600 cursor-pointer">
          <input type="checkbox" checked={soloActivas} onChange={(e) => setSolo(e.target.checked)} className="rounded border-gray-300 text-blue-600" />
          Solo activas
        </label>
      </div>
          ...
          <table className="w-full">
            <thead className="sticky top-0 bg-gray-50/60 z-10">
              <tr className="border-b border-gray-100">
                {["Código", "Nombre", "Descripción", "Cuentas config.", "Estado", ""].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={f.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors">
                    <td className="px-4 py-3 text-[12px] text-gray-800 font-semibold">{f.codigo}</td>
        ...
        <>
          ...
          <div className="fixed top-0 right-0 h-full w-96 bg-white shadow-xl z-50 flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <h2 className="text-[13px] font-semibold text-gray-800">{drawer === "crear" ? "Nueva familia" : "Editar familia"}</h2>
