...
function BuscadorTercero({ value, onChange }: { value: string; onChange: (id: string, label: string) => void }) {
  ...
  function seleccionar(t: Tercero) {
    const etiqueta = `${t.nit} · ${t.razon_social}`;
    setLabel(etiqueta);
    setQuery(etiqueta);
    setAbierto(false);
    onChange(t.id, etiqueta);
  }

  function limpiar() {
    setQuery(""); setLabel(""); setOpciones([]); setAbierto(false);
    onChange("", "");
  }
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex items-center">
        <input
          value={query}
          onChange={e => { setQuery(e.target.value); if (!e.target.value) limpiar(); }}
          onFocus={() => { if (opciones.length) setAbierto(true); }}
          placeholder="Buscar por NIT o nombre…"
          className={`${inp} w-56 pr-6`}
        />
        {query && (
...
export default function MayorPage() {
  ...
  const generar = useCallback(async () => {
    if (!cDesde || !cHasta || !desde || !hasta) return;
    ...
    try {
      ...
      setData(await apiFetch<Mayor>(`/reportes/mayor?${p}`));
    } catch (e: unknown) {
  ...
  async function exportarExcel() {
    ...
    try {
      ...
      const p = new URLSearchParams({ cuenta_desde: cDesde, cuenta_hasta: cHasta, fecha_desde: desde, fecha_hasta: hasta });
      if (terceroId) p.set("tercero_id", terceroId);
      const res = await fetch(`${BASE_URL}/reportes/mayor/excel?${p}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = `mayor_${cDesde}_${cHasta}_${desde}_${hasta}.xlsx`; a.click();
  ...
  function imprimir() {
    const p = new URLSearchParams({ cuenta_desde: cDesde, cuenta_hasta: cHasta, fecha_desde: desde, fecha_hasta: hasta });
    if (terceroId) p.set("tercero_id", terceroId);
    window.open(`/mayor?${p}`, "_blank");
  }

  const totalLineas = data?.cuentas.reduce((s, c) => s + c.lineas.length, 0) ?? 0;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-none border-b border-gray-100 px-5 py-3 bg-white">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input value={cDesde} onChange={e => setCDesde(e.target.value)} placeholder="ej. 1105" className={`${inp} w-28`} />
          </div>
          <div>
            <label className={lbl}>Cuenta hasta</label>
            <input value={cHasta} onChange={e => setCHasta(e.target.value)} placeholder="ej. 1305" className={`${inp} w-28`} />
          </div>
          <div>
          ...
          <div>
            ...
            <BuscadorTercero value={terceroId} onChange={(id) => setTerceroId(id)} />
          </div>
          <button onClick={generar} disabled={loading || !cDesde || !cHasta}
      ...
      <div className="flex-1 overflow-auto px-5 py-4">
        ...
        {!data && !loading && <div className="text-[12px] text-gray-400 mt-8 text-center">Ingresa rango de cuentas y fechas, luego haz clic en Generar</div>}

        {data && (
          <>
            <div className="text-[11px] text-gray-500 mb-3">
              {data.fecha_desde} — {data.fecha_hasta} · Cuentas {data.cuenta_desde} – {data.cuenta_hasta}
              {" · "}{data.cuentas.length} cuenta(s) · {totalLineas} movimiento(s)
            </div>
                ...
                <div key={cuenta.cuenta_codigo} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                  <div className="px-4 py-2 bg-slate-800 text-white flex items-baseline gap-3">
                    ...
                    <span className="ml-auto text-[11px] text-slate-300">
                      Saldo inicial: <span className="font-mono font-bold text-white">{fmtSaldo(cuenta.saldo_inicial)}</span>
                    </span>
                  </div>
                  ...
                  <table className="w-full border-collapse text-[11px]">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="px-2 py-1.5 text-left text-[9px] font-bold uppercase text-gray-500 w-20">Fecha</th>
                        <th className="px-2 py-1.5 text-center text-[9px] font-bold uppercase text-gray-500 w-[120px]">N° Asiento</th>
                        <th className="px-2 py-1.5 text-left text-[9px] font-bold uppercase text-gray-500 max-w-[120px]">Descripción</th>
                        <th className="px-2 py-1.5 text-left text-[9px] font-bold uppercase text-gray-500 w-24">Identificación</th>
                        <th className="px-2 py-1.5 text-left text-[9px] font-bold uppercase text-gray-500 w-48">Nombre</th>
                        <th className="px-2 py-1.5 text-right text-[9px] font-bold uppercase text-gray-500 w-24">Débito</th>
                        <th className="px-2 py-1.5 text-right text-[9px] font-bold uppercase text-gray-500 w-24">Crédito</th>
                        <th className="px-2 py-1.5 text-right text-[9px] font-bold uppercase text-gray-500 w-24">Saldo</th>
                      </tr>
                    </thead>
                    <tbody>
                      {cuenta.lineas.map((ln, i) => (
                        <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-gray-50/50"}>
                          ...
                          <td className="px-2 py-1 text-gray-700 truncate max-w-[120px]" title={ln.descripcion}>{ln.descripcion}</td>
                          <td className="px-2 py-1 font-mono text-[10px] text-gray-600 whitespace-nowrap">{ln.tercero_nit}</td>
                          <td className="px-2 py-1 text-[10px] text-gray-600 truncate max-w-[192px]" title={ln.tercero_nombre}>{ln.tercero_nombre}</td>
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-gray-300 bg-gray-100">
                        <td colSpan={5} className="px-2 py-1.5 font-bold text-[10px] uppercase text-gray-600">Totales</td>
                        <td className="px-2 py-1.5 text-right font-mono text-[10px] font-bold text-gray-900">{fmt(cuenta.totales.debito)}</td>
                        <td className="px-2 py-1.5 text-right font-mono text-[10px] font-bold text-gray-900">{fmt(cuenta.totales.credito)}</td>
                        <td className="px-2 py-1.5 text-right font-mono text-[10px] font-bold text-blue-800">{fmtSaldo(cuenta.totales.saldo_final)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
