...
export default function ComprasAgrupacionPage() {
  usePageTitle();

  const [desde,     setDesde]     = useState(primerDiaMes());
  const [hasta,     setHasta]     = useState(hoyLocal());
  const [agrupPor,  setAgrupPor]  = useState("proveedor");
  ...
  const generar = useCallback(async () => {
    ...
    try {
      ...
      setData(await apiFetch<Reporte>(`/reportes/compras-agrupacion?${p}`));
    } catch (e: unknown) {
  ...
  async function exportarExcel() {
    ...
    try {
      ...
      const p = new URLSearchParams({ fecha_desde: desde, fecha_hasta: hasta, agrupar_por: agrupPor });
      const res = await fetch(`${BASE_URL}/reportes/compras-agrupacion/excel?${p}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url; a.download = `compras_${agrupPor}_${desde}_${hasta}.xlsx`; a.click();
  ...
  function imprimir() {
    const p = new URLSearchParams({ fecha_desde: desde, fecha_hasta: hasta, agrupar_por: agrupPor });
    window.open(`/compras-agrupacion?${p}`, "_blank");
  }

  const mostrarCantidad = agrupPor !== "proveedor";
  const mostrarId = agrupPor !== "familia";
  const idHeader = agrupPor === "proveedor" ? "NIT" : "Código";
