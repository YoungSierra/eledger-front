...
export default function SaldosProveedoresPage() {
  ...
  const puedeCrearComprobante = useHasRuta("/dashboard/cxp/comprobantes");
  const [fechaCorte, setFechaCorte] = useState(hoyLocal());
  ...
  const cargar = useCallback(async (fecha: string) => {
    setLoading(true);
    try {
      const res = await apiFetch<ResumenResponse>(`/cxp/resumen?fecha_corte=${fecha}`);
      setData(res);
    } finally {
      setLoading(false);
    }
  ...
  async function exportarExcel() {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/cxp/resumen/excel?fecha_corte=${fechaCorte}`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      const blob = await res.blob();
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement("a");
      a.href = url; a.download = `saldos_cxp_${fechaCorte}.xlsx`; a.click();
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-4 shrink-0">
        <div>
          ...
          <p className="text-[12px] text-gray-400 mt-0.5">Saldos pendientes de pago agrupados por antigüedad</p>
        </div>
        <div className="flex items-center gap-2 no-print">
          ...
          <button onClick={() => window.open(`/cxp-saldos?fecha_corte=${fechaCorte}`, "_blank")} disabled={!data || loading}
          ...
          <table className="w-full min-w-[760px] text-[12px]">
            <thead className="sticky top-0 bg-white z-10 border-b border-gray-100">
              <tr>
                ...
                <th className="px-3 py-2.5 text-left text-[10px] font-bold uppercase tracking-wide text-gray-400">Proveedor</th>
                ...
                <tr key={item.tercero_id} className="hover:bg-gray-50/60 transition-colors">
                  <td className="px-3 py-2.5">
                    {puedeCrearComprobante ? (
                      <button
                        onClick={() => router.push(`/dashboard/cxp/comprobantes?tercero_id=${item.tercero_id}&tercero_display=${encodeURIComponent(`${item.tercero_nit ?? ""} — ${item.tercero_nombre ?? ""}`)}`)}
                        title="Crear comprobante de pago para este proveedor"
