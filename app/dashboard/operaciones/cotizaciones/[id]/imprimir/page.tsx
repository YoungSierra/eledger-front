...
export default function ImprimirPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const seccionesConLineas = SECCIONES.filter(({ key }) =>
    cot.lineas.some((l) => l.seccion === key)
  );
    ...
    <div style={{ height: "100%", overflowY: "auto", background: "#fff" }}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media print {
          ...
          div[style*="min-h-screen"] > div:first-child { display: none !important; }
        }
      `}</style>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px 40px", fontSize: 11, color: "#1e293b", lineHeight: 1.5 }}>
        ...
        {seccionesConLineas.map(({ key, label }) => {
          ...
          let subtotal = 0;
          filas.forEach((l) => { subtotal += l.total_venta; });
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                <thead>
                  <tr style={{ background: "#eff6ff" }}>
                    ...
                    <th style={{ textAlign: "center", padding: "5px 8px", fontWeight: 600, color: "#1e40af", borderBottom: "1px solid #bfdbfe", whiteSpace: "nowrap" }}>Tipo cálculo</th>
                ...
                <tbody>
                  {filas.map((l, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f8fafc" }}>
                      ...
                      <td style={{ padding: "5px 10px", textAlign: "right", color: "#475569", borderBottom: "1px solid #f1f5f9" }}>
                        {l.tipo_calculo === "PORCENTAJE" ? `${l.valor_unitario}%` : fmt(l.valor_unitario)}
                      </td>
                  ...
                  {/* Subtotal sección */}
                  <tr style={{ background: "#eff6ff" }}>
                    <td colSpan={3} style={{ padding: "5px 10px", fontWeight: 700, color: "#1e40af", fontSize: 10 }}>
                      Subtotal {label}
                    </td>
                    <td style={{ padding: "5px 10px", textAlign: "right", fontWeight: 700, color: "#1e40af" }}>
                      COP {fmt(subtotalCOP, 0)}
                    </td>
                    <td />
                  </tr>
                </tbody>
        ...
        {/* Footer */}
        <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: 16, marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div style={{ fontSize: 9, color: "#94a3b8" }}>
            <div>Documento generado por el sistema _eLedger</div>
            <div>Universal Cargo Colombia S.A.S · NIT 901.702.367</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ borderTop: "1px solid #cbd5e1", width: 160, paddingTop: 4, fontSize: 9, color: "#64748b" }}>
              Firma autorizada
            </div>
          </div>
        </div>
